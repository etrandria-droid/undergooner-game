...
rulesBtn.style.cssText = btnStyle + "top:16px;right:16px;";
settingsBtn.style.cssText = btnStyle + "top:56px;right:16px;";
...
rulesBtn.onclick = () => {
  ...
  settingsPanel.style.display = "none"; // fermer l'autre
};
...
settingsBtn.onclick = () => {
  ...
  if (isOpen) {
    ...
    rulesPanel.style.display = "none"; // fermer l'autre
  }
};
...
socket.on("updateLobby", (players) => {
  ...
  // 🔥 Règles visible pour tous, paramètres visible pour tous (lecture seule pour invités)
...
// 🔥 Remplacer les alert() par des notifs custom
socket.on("nameTaken", () => {
...
socket.on("backToLobby", () => {
  ...
  localStorage.removeItem("players");const socket = io();
